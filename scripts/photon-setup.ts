...
async function runSetup(args: Args, fetchFn: FetchFn = fetch, deps: SetupDeps = {}): Promise<number> {
  ...
  p.outro(
    [
      ...
      phone
        ? `  3. Wire your DM to an agent:  npx tsx scripts/init-first-agent.ts --channel photon --user-id photon:${phone} --platform-id ${phone} --display-name "You"`
