...
Dependency install (host `pnpm install --frozen-lockfile` + container `bun install --frozen-lockfile`) is handled by the automatic startup/update script. Bun installs to `~/.bun/bin` (add to `PATH` or call `~/.bun/bin/bun` directly). The notes below are the non-obvious startup caveats for running/testing the app in this VM.

- **Docker is required and must be started manually — there is no systemd.** The host (`pnpm run dev`) calls `ensureContainerRuntimeRunning()` and exits fatally if `docker info` fails. Start the daemon in the background before running the host, e.g. in a tmux session: `sudo dockerd > /tmp/dockerd.log 2>&1 &`, then make the socket usable without sudo (`sudo chmod 666 /var/run/docker.sock`; the `ubuntu` user is already in the `docker` group). Docker 29 is installed with `storage-driver: fuse-overlayfs` and `features.containerd-snapshotter: false` in `/etc/docker/daemon.json` — both are required for image builds to work in this VM; do not remove them.
- **Build the agent image before exercising agents:** `./container/build.sh` (tags `nanoclaw-agent-<slug>:latest`). Host-spawned sessions mount fresh `src/` over the image, so host edits don't need a rebuild; rebuild only when the Dockerfile or container deps change.
- **Upgrade tripwire blocks startup on a fresh checkout.** `enforceUpgradeTripwire()` refuses to boot unless `data/upgrade-state.json` (gitignored) records the current `package.json` version. On a fresh clone / after a version bump, stamp it once before `pnpm run dev`: `pnpm exec tsx scripts/upgrade-state.ts set`. This is the sanctioned coding-agent remedy printed in the boot error itself.
- **A full agent reply needs OneCLI credentials, which are not present by default.** With no `ONECLI_API_KEY`, the message pipeline works end-to-end (router → session `inbound.db` → container spawn) but `container-runner.ts` fails at `onecli.ensureAgent` with a 401 against `https://api.onecli.sh` and the host-sweep retries. To get real replies, either supply an `ONECLI_API_KEY` for a vault holding an Anthropic credential, or run a local OneCLI gateway (`/init-onecli`). Provider `claude` is baked in; there is a `mock` provider in the agent-runner for tests, but the host still requires the OneCLI gateway to be applied before any container spawn.
- **No channel tokens are needed for local testing.** The `cli` channel ships in trunk and is always on. Bootstrap a CLI-wired agent with `pnpm exec tsx scripts/init-cli-agent.ts --display-name "<name>"`, then send messages via `pnpm run chat "<text>"` (talks over `data/cli.sock`). Inspect what reached the container with the in-tree query wrapper against `data/v2-sessions/<agent-group>/<session>/inbound.db`.
- Standard lint/test/build/run commands live in the **Development** and **build-and-runtime** sections above; the repo CI gate is `format:check` + host/container `tsc` + `vitest` + `bun test` (note: `pnpm run lint` is not part of CI and has pre-existing errors on a clean checkout).
