...
- **First-party Photon (iMessage) channel + zero-friction setup wizard.** `photon` is a native channel adapter (`src/channels/photon.ts`) shipped in trunk and wired into the channel barrel — it connects NanoClaw to iMessage through [Photon](https://photon.codes)'s managed Spectrum platform over the `spectrum-ts` gRPC stream, in-process on the host (no Mac relay, no webhook, no sidecar). It stays dormant until `PHOTON_PROJECT_ID` + `PHOTON_PROJECT_SECRET` exist, so default installs are unaffected. `/add-photon` installs the pinned SDK and runs `scripts/photon-setup.ts`, which performs Photon's device-login flow and auto-provisions the project, secret, phone user, and iMessage line — writing creds to `.env` with no dashboard clicking. Both directions, markdown, attachments, tapback reactions, typing, and `ask_user_question` (via slash-command replies) are supported. See [docs/photon.md](docs/photon.md).
